...
✅ 2026-09-19：**`official_standings` を統一するのではなく、別キー `source_standings` を並べて足した。**
   ...
   ⚠️ `rank` を持つのは**11リーグ**（北海道・兵庫・三重・奈良・沖縄・大阪・和歌山
      ＋長野・京都・福島・愛媛）。2026-09-18 にソースを読んで「4本」と数えたが、
      **実物は7本**だった（読むだけでは外れる）。残りは読み手が順位の列を standings に
      入れていなかっただけで、2026-09-20 に長野・京都・福島・愛媛を直して11リーグになった。
