...
✅ 2026-09-19：**`official_standings` を統一するのではなく、別キー `source_standings` を並べて足した。**
   ...
   ⚠️ `rank` を持つのは**9リーグ**（北海道・兵庫・三重・奈良・沖縄・大阪・和歌山
      ＋長野・京都）。2026-09-18 にソースを読んで「4本」と数えたが、**実物は7本**だった
      （読むだけでは外れる）。長野・京都は読み手が順位の列を standings に入れていなかったので、
      2026-09-20 に `read_nagano` / `read_kyoto` を直して9リーグになった。
