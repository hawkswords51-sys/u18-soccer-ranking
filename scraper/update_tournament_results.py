...
def canon(name: str) -> str:
    ...
    for suf in ("高等学校", "高校", "高等部"):
        if n.endswith(suf) and len(n) > len(suf):
            n = n[: -len(suf)]
            break
    # [2026-09-25] 「◯◯中学・高等学校」は上で「高等学校」だけが落ち、「◯◯中学・」と中黒で切れる
    #   （東京の「文化学園大学杉並中学・高等学校」が md に「文化学園大学杉並中学・」のまま5か所載った）。
    #   末尾が「・」のときだけ「・」と、続く「中学校／中学」を落とす。md の旧表記
    #   「文化学園大学杉並中学・」も同じ「文化学園大学杉並」になるので、照合は切れない。
    #   ⚠️ 「中等教育学校」は末尾が「・」にならないので、この処理には掛からない（上の⚠️を参照）。
