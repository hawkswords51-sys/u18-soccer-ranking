...
# 退行防止の例外
# ----------------------------------------------------------------------------
# ここに県を入れると、その県だけ「試合数が減っても上書きする」ようになる。
# **一時的な措置にしか使わないこと。** 入れっぱなしにすると、その県だけ退行ガードが
# 永久に無効になる。2026-09-07に沖縄が那覇の異常値（試合が減るのに勝点が増える）で
# 守られたのは、このガードのおかげ。
#
# ⚠️ **例外を足したら、外す条件も一緒に書くこと。** 例外は書いた本人しか覚えていないので、
#    放っておくと居座る。有効な例外は audit_pref_freshness.py の⚪️情報行に出るので、
#    そこに何か出ていたら「まだ外せないか」を確認する。
#
# 過去の例外（どちらも 2026-09-07 に役目を終えて削除済み）:
#   okayama   … teams.json に junior-soccer 由来の破損値（10チームリーグで26試合・
#               1勝17分8敗）が入っており、公式へ移す際に試合数が減った。破損値の修正で
#               あって退行ではなかった。→ 公式移行が完了し、減らなくなったので削除。
#   hiroshima … junior-soccer が**公式に存在しない試合**「サンフレセカンド 5-1 銀河学院」
#               （日付が空）を登録しており、除去すると2チームが 10→9試合 に減った。
#               存在しない試合が消えるだけで退行ではなかった。→ 一度同期して teams.json も
#               9試合になり、減らなくなったので削除。
