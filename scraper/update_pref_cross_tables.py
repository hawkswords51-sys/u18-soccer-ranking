    ...
    ⚠️⚠️ ただし **行順＝順位とは限らない**（2026-09-19 実測。設計時の想定が外れた）。
       ...
       **`rank` 列を持つリーグでも行順とは限らない**（兵庫は先頭が23点で3番目が24点）。
       → **同着を戻す材料になるのは `rank` の値だけ**で、並び順は使えない。
         `rank` を持つのは11リーグ（北海道・兵庫・三重・奈良・沖縄・大阪・和歌山
         ＋長野・京都・福島・愛媛。後の4つは2026-09-20に読み手を直して入った）。
       📌 `official_standings` は勝点→得失点差→得点→名前で並べ直すので同着が必ず割れる。
          その復元は**読み手が `rank` を返しているリーグでしかできない**（＝上の11）。
...
MIGRATED_TO_OFFICIAL = {
    ...
    "pref-saitama-1",    # sfa2.jp（高体連サッカー専門部の前・後期日程・結果PDF＋S1星取表PDF）2026-09-27追加
}
...
# このスクリプトが取りに行かない県 → 理由と、見張りの扱い。
# ⚠️ 2026-09-15：このスクリプトは Actions から外し、Macから週1で手動実行する専用になった
#    （junior-soccer が Actions のIPを403で弾くため。update_rankings.yml のコメント参照）。
# ⚠️⚠️ 2026-09-18：**「取り込まない」と「見張らない」は別の話**。ここを1つの値で兼ねていたため、
#    埼玉を手動取り込みの対象に戻したいのに、除外を外さないと🟡Aが鳴らせない状態になっていた。
#    `watch` で割る。**このスクリプトは触らないが、人が手で取り込むので止まったら鳴らす**、が表せる。
MANUAL_IMPORT_EXCLUDED = {
    # 2026-09-27：埼玉は sfa2.jp（高体連サッカー専門部）へ移行し、fetch_pref_official.py が毎日取り込む
    #   （MIGRATED_TO_OFFICIAL で手前から外れる）ので、ここから消した。**表は空だが仕組みは残す。**
}
