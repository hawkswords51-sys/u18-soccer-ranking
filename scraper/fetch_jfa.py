...
LEAGUE_ALIASES = {
    ...
    "prince-kyushu-2": {
        "九州国際大学付属高校": "九州国際大付",
        "大津高校3rd": "大津3rd",
        # [2026-09-08] JFA表記が半角アンダースコア、こちらは半角スペース。
        #   _norm() はスペースは消すがアンダースコアは消さないので名寄せできず、
        #   2026-09-06〜08 の3日間このリーグ丸ごとが据え置き（koko据え置き）になっていた。
        #   ⚠️ _norm() で「_」を一律にスペースへ置換する直し方はしない。
        #      プリンス12リーグを全走査してアンダースコアはこの1件だけと確認済み。
        #      1件のために全チーム名の正規化規則を変えると影響範囲が読めない。
