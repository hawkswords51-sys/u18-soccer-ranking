...
def build_faqs(pref_name, teams, pref_id=None):
    ...
    # Q4: 更新頻度
    # [2026-09-05] 実態に合わせて修正。以前は「毎日 9:00 頃」「最短で翌朝」と書いていたが、
    # 実際は 7:00 JST と 22:00 JST の1日2回（update_rankings.yml の cron）。
    # 夜の回が増えたので、出典が更新されていれば当日の夜に反映されることもある。
    ...
    a4 = (
        ...
        "週末の試合結果は、出典が更新され次第、最短でその日の夜に順位表へ反映されます。"
    )
...
<section class="lp-section">
  ...
  <p>
    ...
    全国の頂点に立つ<strong>プレミアリーグ</strong>（EAST・WEST 各12チーム）、その下の
    <strong>プリンスリーグ</strong>（全国9地域）、そして各都道府県の<strong>県リーグ</strong>（1部・2部など）という
    ピラミッド型の階層構造になっています。各カテゴリの間には昇格・降格があり、
    県リーグで結果を残したチームが地域のプリンスリーグへ、プリンスリーグの上位がプレミアリーグへと
    駆け上がっていく仕組みです。
  </p>
  <p>
    ただし公式サイトでは、これらのリーグの順位が<strong>カテゴリごとに別々の表でしか確認できません</strong>。
    そのため「__PREF_NAME__のチームは、いま全国のピラミッドのどのあたりにいるのか」を知るには、
    複数のページを見比べる必要がありました。このページでは、リーグの格と各リーグ内の順位を加味して
    __PREF_NAME__のチームを1つの表にまとめ、カテゴリを越えた<strong>県内総合順位</strong>として
    一覧できるようにしています。上の順位表で気になったチームがあれば、チーム名から詳細ページへ進むと、
    創部の歴史・主なOB・近年の全国大会成績まで確認できます。
  </p>
</section>
...
def generate_page(pref, all_prefs):
    ...
    if neighbors:
        ...
        # 北海道のように単独で1地域ブロックを構成する県は「近隣」が存在しない。
        # 「情報を準備中」と出すと未完成のページに見えるため、案内文に置き換える。
        neighbor_links = (
            '          <p style="margin:0;">この都道府県は単独で1つの地域ブロックを'
            '構成しているため、同一ブロック内の他県はありません。'
            '他のエリアの順位は<a href="/">全国47都道府県の一覧</a>、'
            '広域リーグの順位は<a href="/leagues/">リーグ一覧</a>からご覧いただけます。</p>'
        )
