...
def canonical_club(club: str) -> str:
    """表示用の正式名。JFAがトップチーム名で登録している所属を、CLUB_ALIASES の
       U-18表記に置き換えて返す（例: RB大宮アルディージャ → RB大宮アルディージャU18）。
       ユース側に詳細ページが無く県ページ扱い（pref）になる選手でも、表示だけは
       U-18名にそろえるために使う。該当が無ければそのまま返す。
       ※現所属が本当にトップ/大学/プロの選手は origin を付けるため、この変換は
         origin の無い（＝ユース年代所属の）選手にだけ適用する（呼び出し側で制御）。"""
    base = _norm(club)
    for k, v in CLUB_ALIASES.items():
        if _norm(k) == base:
            return v
