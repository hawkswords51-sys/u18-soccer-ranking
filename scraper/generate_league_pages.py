...
from league_contents import (
    ...
    TACTICAL_PRINCE_TOHOKU, WATCHING_PRINCE_TOHOKU,
    TACTICAL_PRINCE_KANTO_1, WATCHING_PRINCE_KANTO_1,
    TACTICAL_PRINCE_KANTO_2, WATCHING_PRINCE_KANTO_2,
    TACTICAL_PRINCE_HOKUSHINETSU_1, WATCHING_PRINCE_HOKUSHINETSU_1,
    TACTICAL_PRINCE_HOKUSHINETSU_2, WATCHING_PRINCE_HOKUSHINETSU_2,
    TACTICAL_PRINCE_TOKAI, WATCHING_PRINCE_TOKAI,
    TACTICAL_PRINCE_KANSAI_1, WATCHING_PRINCE_KANSAI_1,
    TACTICAL_PRINCE_KANSAI_2, WATCHING_PRINCE_KANSAI_2,
    TACTICAL_PRINCE_CHUGOKU, WATCHING_PRINCE_CHUGOKU,
    TACTICAL_PRINCE_SHIKOKU, WATCHING_PRINCE_SHIKOKU,
...
# === リーグ全史（歴代順位・名称/方式の変遷・プレミア昇格の歴史）の読み込み ===
LEAGUE_HISTORY_FULL_PATH = Path(__file__).parent.parent / "data" / "league_history_full.yml"


def load_league_history_full():
    """data/league_history_full.yml を読み、slug → 全史データ の辞書にして返す。

    1つのリーグ系統（例: 北信越）のデータを、1部・2部の両方のページから
    参照できるように slugs: を展開しておく。
    """
    if not LEAGUE_HISTORY_FULL_PATH.exists():
        return {}
    try:
        with open(LEAGUE_HISTORY_FULL_PATH, encoding="utf-8") as f:
            raw = yaml.safe_load(f) or {}
    except Exception as e:
        print(f"⚠️ league_history_full.yml 読み込みエラー: {e}")
        return {}

    by_slug = {}
    for family_id, data in raw.items():
        if not isinstance(data, dict):
            continue
        data = dict(data)
        data["_family_id"] = family_id
...
LEAGUE_DEFS = {
   ...
   "プリンスリーグ北信越1部": (
        ...
        "プリンスリーグ 北信越 1部",
        "prince",
        "高円宮杯 JFA U-18 サッカープリンスリーグ 北信越 1部。北信越地方（新潟・富山・石川・福井・長野）の上位チームが参加。",
    ...
    "プリンスリーグ北信越2部": (
        ...
        "プリンスリーグ 北信越 2部",
        "prince",
        "高円宮杯 JFA U-18 サッカープリンスリーグ 北信越 2部。北信越地方の準上位チームが参加。",
...
def build_faqs(label, teams, slug, category):
    ...
    # [2026-09-05] 実態に合わせて修正。以前は「毎日 9:00 頃」と書いていたが、
    # 実際は 7:00 JST と 22:00 JST の1日2回（update_rankings.yml の cron）。
    # [2026-09-08] 時刻そのものを書くのをやめた。GitHub Actions のスケジュール実行は
    # 数時間遅れることがあり（2026-09-07 22:00 JST の回が実際は翌 02:59 JST に実行）、
    # 分単位で書くと読者に対して嘘になるため。「毎日2回（朝・夜）」までにとどめる。
    a4 = (
        "本サイトの順位データは、<strong>毎日2回（朝・夜）自動で更新しています。</strong>"
        "JFA（日本サッカー協会）公式サイト・各地域サッカー協会の最新データを反映しています。"
        "プレミアリーグはJFA公式の試合結果を直接取得しているため、"
        "夕方に終わった試合もその日の夜には順位表に反映されます。"
    )
...
def _chronicle_team_cell(team):
    ...
    if slug:
        inner = (
            f'<a href="/teams/{html_escape(str(slug))}/" '
...
def render_league_chronicle_html(slug, label):
    ...
    データは data/league_history_full.yml。該当データが無いリーグでは空文字を返すので、
    北信越以外のリーグページは今まで通り何も表示されない。
    """
    data = LEAGUE_HISTORY_FULL.get(slug)
    if not data:
        return ""
    ...
    if intro:
        parts.append(
            f'<p style="line-height:1.9; margin:0 0 12px 0;">{html_escape(str(intro).strip())}</p>'
        )
    intro_slug = (data.get("intro_by_slug") or {}).get(slug)
    if intro_slug:
        parts.append(
    ...
    if name_history:
        rows = "".join(
            ...
            f'{html_escape(str(n.get("period","")))}</span>'
            f'<span>{html_escape(str(n.get("name","")))}</span></li>'
            for n in name_history
        )
        parts.append(
            f'<h3 style="margin:28px 0 10px 0;">大会名称の変遷</h3>'
    ...
    if eras:
        parts.append('<h3 style="margin:32px 0 10px 0;">歴代順位</h3>')
        parts.append(
            f'<p style="font-size:.9em; color:{muted}; margin:0 0 16px 0;">'
    ...
    for era_index, era in enumerate(eras):
        ...
        head += "".join(
            f'<th style="padding:6px 10px; white-space:nowrap;">{html_escape(str(c))}</th>'
            for c in cols
        )

        body = ""
        for season in era.get("seasons", []):
            season_rows = season.get("rows", [])
            span = len(season_rows)
            for i, row in enumerate(season_rows):
                ...
                if has_div:
                    body += (
                        f'<td style="padding:6px 10px; white-space:nowrap; color:{muted};">'
        ...
        parts.append(
            '<div style="overflow-x:auto; -webkit-overflow-scrolling:touch; margin:0 0 6px 0;">'
...
def render_related_leagues_html(current_slug, current_category):
    ...
    items = []
    for league_name, (slug, label, category, _desc, _overview, _tactical, _watching) in LEAGUE_DEFS.items():
...
__PREF_DISTRIBUTION__
      ...
      <!-- ★ 新規追加：戦術的な見どころ -->
      <section class="lp-section lp-tactical">
        <h2><i class="fas fa-chess"></i> 戦術的な見どころ</h2>
__TACTICAL_POINTS__
      </section>

      <!-- ★ 新規追加：観戦のポイント -->
      <section class="lp-section lp-watching">
        <h2><i class="fas fa-binoculars"></i> 観戦のポイント</h2>
__WATCHING_POINTS__
      </section>
...
__RELATED_LEAGUES__
      ...
      <section class="lp-section">
        ...
        <ul class="lp-related-links">
          ...
          <li><a href="/blog/posts/premier-league-guide-2026/">プレミアリーグとは？仕組み・昇格降格・ファイナルを解説</a></li>
...
def generate_league_page(league_name, slug, label, category, description, season_overview, teams, tactical_points, watching_points):
    ...
    if chronicle_html:
        ...
        # 全史は1部・2部で共通の内容だから。
        family = LEAGUE_HISTORY_FULL.get(slug, {})
        chronicle_label = family.get("label") or label
        chronicle_section = (
            '      <section class="lp-section lp-chronicle" id="chronicle">\n'
            f'        <h2><i class="fas fa-landmark"></i> {html_escape(chronicle_label)}の歴史</h2>\n'
            f'{chronicle_html}\n'
            '      </section>'
        )
    else:
...
"""
    ...
    return (
        ...
        .replace("__WATCHING_POINTS__", watching_points)
    )
...
<body>
  ...
  <main class="main-content">
    <div class="container">
      ...
      <p class="lp-intro">
        高円宮杯JFA U-18 サッカープレミアリーグの<strong>ファイナル（決勝戦）</strong>の歴代結果をまとめています。
      ...
      {ranking_html}
      <section class="lp-section">
        <h2><i class="fas fa-circle-question"></i> プレミアファイナルとは——「もうひとつの日本一」</h2>
        <p style="line-height:1.9;">
          高校年代の「日本一」と聞くと、多くの人は冬の全国高校サッカー選手権を思い浮かべます。しかしもうひとつ、
          <strong>1年間のリーグ戦を勝ち抜いた者どうしが激突する日本一決定戦</strong>があります。それがプレミアリーグ ファイナルです。
          全国を東西に分けたプレミアリーグEAST・WESTの<strong>各1位チームが、リーグ最終節の後（例年12月中旬）に一発勝負</strong>で対戦し、
          その年の高校年代リーグ王者を決めます。
        </p>
        <p style="line-height:1.9;">
          選手権が「負けたら終わり」のトーナメントで勝ち上がった代表校の頂点を決めるのに対し、ファイナルは
          <strong>4月から12月まで約20試合を戦い抜いた末の1位どうしの対戦</strong>。Jクラブユースと高体連（高校の部活）の強豪が
          同じ土俵で戦うプレミアリーグの性格上、「ユース最強 vs 高校最強」の構図になる年も多く、
          スカウトや育成関係者が最も注目する一戦とも言われます。出場権を争う下部リーグとの入れ替えは
          <a href="/tournaments/promotion-playoff-2026/">プレミアリーグ参入戦</a>で行われます。
        </p>
        <details style="margin:10px 0;"><summary style="cursor:pointer;font-weight:600;">Q. プレミアファイナルに出場するにはどうすればよいですか？</summary>
        <p style="margin:10px 0 4px;line-height:1.8;">プレミアリーグEASTまたはWEST（各12チーム）で年間1位になることが唯一の条件です。EAST・WESTの現在の順位は下の関連リーグからご覧いただけます。</p></details>
        <details style="margin:10px 0;"><summary style="cursor:pointer;font-weight:600;">Q. 選手権やインターハイとは何が違うのですか？</summary>
        <p style="margin:10px 0 4px;line-height:1.8;">選手権・インターハイは高体連（高校の部活）だけが出場するトーナメントですが、プレミアリーグにはJクラブのユースチームも参加します。ファイナルは、その混成リーグを勝ち抜いた東西王者による「リーグ戦の日本一決定戦」です。</p></details>
        <details style="margin:10px 0;"><summary style="cursor:pointer;font-weight:600;">Q. 直近のファイナルの結果は？</summary>
        <p style="margin:10px 0 4px;line-height:1.8;">上の歴代結果一覧をご覧ください。2026年大会のカードはリーグ最終節（12月）後に確定し、<a href="/tournaments/premier-final-2026/">プレミアファイナル2026特設ページ</a>で更新します。</p></details>
      </section>
      <section class="lp-section">
        <h2><i class="fas fa-trophy"></i> 関連リーグ</h2>
...
def generate_index_page(teams_by_league):
    ...
    premier_cards = []
    prince_cards = []
    for league_name, (slug, label, category, description, season_overview, tactical_points, watching_points) in LEAGUE_DEFS.items():
...
def main():
    ...
    for league_name, (slug, label, category, description, season_overview, tactical_points, watching_points) in LEAGUE_DEFS.items():
        ...
        html = generate_league_page(league_name, slug, label, category, description, season_overview, teams, tactical_points, watching_points)
