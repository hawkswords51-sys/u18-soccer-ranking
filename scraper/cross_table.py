...
NO_HOME_AWAY_SLUGS: set[str] = {
    ...
    "pref-ibaraki-1",
    # ↓ 2026-09-18。47県目として取り込んだ北海道FAリーグ。**②は未判定**。
    #   既定は「出さない」なので、判定するまでは出さない（岐阜で決めた原則）。
    #   ⭐️ 会場は42試合すべてに入っていて11種類あるので、**②で判定できる状態**にはなっている。
    #   ⚠️ 公式PDFに「リーグ表の上段：ホーム 下段：アウェー」という注記があるが、
    #      **注記は証拠にならない**（福島の `HOME` 列が中身を伴っていなかった前例）。会場で確かめること。
    "pref-hokkaido-1",
    # ⚠️ 千葉（左43/43＝100%・無関係0）と東京（92%）は**本物**なのでここに入れない。
