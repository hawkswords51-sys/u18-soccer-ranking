...
NO_HOME_AWAY_SLUGS: set[str] = {
    ...
    # ⚠️ 埼玉は当初「③が 2ペア中1 で標本が小さい」ため、**②が確かめられないこと
    #    （junior-soccer に会場が無い）だけ**を根拠に入れた。
    #    ⭐️ 2026-09-18に47→54試合を取り込んだ結果、**③は9ペア中5（56%）＝過半数**になり、
    #       ③単独でも「H/Aではない」と言える水準になった。根拠が1つ増えただけで結論は変わらない。
    #    📌 `NO_HOME_AWAY_SLUGS` は表示設定なので、取り込みの運用（別運用 → 週1の手動取り込み）とは無関係。
    "pref-saitama-1",
    # ------------------------------------------------------------------
