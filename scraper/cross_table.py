...
NO_HOME_AWAY_SLUGS: set[str] = {
    ...
    "pref-ehime-1",      # 愛媛県1部（2026-09-16）。会場10種すべてが公共施設＝学校名の会場が0のセントラル開催
    "pref-kyoto-1",      # 京都府1部（2026-09-16）。学校のグラウンドでも左右が逆・自校が出ない試合があり会場＝ホームでない
    "pref-ishikawa-1",   # 石川県1部（2026-09-17）。H7/A7・28ペアすべて入れ替わりでも会場は56試合中45が中立（左1・右4・無関係6）
    "pref-osaka-1",      # 大阪府1部（2026-09-17）。H9/A9・全ペア入れ替わりでも会場は左15・右20・無関係28＝①③は根拠にならない
    "pref-wakayama-1",   # 和歌山県1部（2026-09-17）。ホーム試合数が最大6差・25ペア中7が同じ側・90試合中59が中立会場
    ...
    "pref-hiroshima-1",  # 10ペア中10が同じ側（SportsOnline）→ 2026-09-17に②でも否定（下）
    "pref-kagawa-1",     # 10ペア中10が同じ側（GoalNote）
    "pref-tochigi-1",    #  9ペア中 9が同じ側（LSIN）→ 2026-09-17に②でも否定（下）
    "pref-saga-1",       #  8ペア中 8が同じ側
    "pref-shimane-1",    #  7ペア中 7が同じ側（SportsOnline）→ 2026-09-17に②でも否定（下）
    "pref-tottori-1",    #  6ペア中 6が同じ側（GoalNote）
    "pref-oita-1",       #  5ペア中 5が同じ側（PDF）
    # ↓ 2026-09-17。**junior-soccer 出典（有志入力）で、左右に意味があるという根拠が無い。**
    #   ⚠️ 決め手は「**②（会場の持ち主）が原理的に確かめられない**」こと。この出典のデータには会場が無いので、
    #      「H/Aは証明されたときだけ出す」の原則に照らすと**証明される見込みが無い**＝出さない。
    #   ③（2試合あるペアのうち両巡とも同じ側だったペア）の実測。本物のホーム/アウェイなら0になる。
    "pref-miyagi-1",     # 24ペア中19が同じ側（79%）
    "pref-kagoshima-1",  # 21ペア中13が同じ側（62%）
    "pref-shizuoka-1",   # 15ペア中10が同じ側（67%）
    "pref-osaka-2a",     #  7ペア中5が同じ側（71%）
    "pref-osaka-2b",     #  8ペア中7が同じ側（88%）
    "pref-osaka-2c",     #  7ペア中5が同じ側（71%）
    # ✅ 埼玉（pref-saitama-1）は 2026-09-27 にここから外した（Kei判断）。sfa2.jp の日程・結果PDFに
    #    「ホーム」「アウェイ」の列見出しがあり、①全10チームがH9/A9 ②45ペアすべて2試合で左右が入れ替わる（同じ側0）
    #    ③会場の持ち主が分かる7校はホーム戦をすべて自校で開催（成徳深谷・狭山ヶ丘・埼玉栄の3校は自校の会場が無く、
    #    ホーム戦17試合が相手校のグラウンド＝主管がホームの扱い）。以前ここに入れていた理由（junior-soccer 出典に会場が無い）は
    #    出典が変わったので無効。
    # ------------------------------------------------------------------
    # ↓ 2026-09-17。**会場を `matches[].venue` に保存できたので、②で判定した**
    #   （素材＝運営フォルダ「会場の一覧6リーグ_H_A判定2の素材_2026-09-17.md」／
    #     判定＝「6リーグのH_A判定_結果_2026-09-17.md」）。
    #   ⭐️ 会場で判定できた7リーグのうち**「本物」は東京T1だけ**だった。
    #      県リーグでは本物のホーム/アウェイのほうが例外で、「既定は出さない」が実測で裏づけられた。
    #
    #   ⚠️ ②は**2段階**で読む。「無関係が0」だけで白にしないこと。
    #      第1段：学校/クラブ名の会場で「両チームとも無関係」が多数 → セントラル開催で確定（＝下の【A】）
    #      第2段：無関係が0なら「主催者はいる」。次に**左がその主催者か**を見る
    #             90%以上＝本物（東京92%）／60〜70%台＝証明できず（＝下の【B】）／右が多い＝否定
    #      「主催者がいる」と「左が主催者」は別の話。東京だけ見ていると第2段に気づけない。
    # ------------------------------------------------------------------
    # 【A】学校/クラブ名の会場なのに**両チームとも無関係**な試合が多数＝持ち回りのセントラル開催。
    #      ⚠️ 栃木・広島・島根は③（両巡とも同じ側）を根拠に2026-09-16から既に上に入っている。
    #         ここは**②という独立した根拠が同じ結論を支持した**という記録で、重複登録ではない。
    #         ・栃木 … 会場6種が**すべて学校/クラブ名で中立0**なのに、53試合中24が両チームとも無関係
    #         ・広島 … 34試合中11が両チームとも無関係（20が中立）
    #         ・島根 … 学校名の会場は8試合だけ（27が中立）で、左2・**右5**＝右のほうが多い
    "pref-nagano-1",     # 長野1部。学校名の会場42試合のうち**24が両チームとも無関係**（左10・右8・中立14）
    # 【B】⚠️ **セントラル開催ではない**（両チームとも無関係＝0）。学校のグラウンドは必ず当事者のもの。
    #      ＝「主催者はいる」ことは言える。**しかし「左＝その主催者」が言えない。**
    #      左が持ち主なのは熊本65%・宮崎71%で、当てずっぽうの50%より高いだけ。
    #      `H` のバッジは「この試合はホーム戦です」と読者に断言するので、3回に1回まちがう表示は出せない。
    #      📌 これは「**H/Aではない**」ではなく「**証明できなかった**」。
    #         出典が左右に主催を入れている可能性は残っている。出典が会場や主催の列を増やしたら再判定する。
    #         （例：秀岳館人工芝Gで秀岳館が右／宮崎日本大学高校第2Gで宮崎日大が右）
    "pref-kumamoto-1",   # 熊本1部。学校名の会場23試合：左15（65%）・右8・**無関係0**（39が中立）
    "pref-miyazaki-1",   # 宮崎1部。学校名の会場21試合：左15（71%）・右6・**無関係0**（63が中立）
    # ↓ 2026-09-17。③の「過半数が同じ側」で確定。②（会場）は見ていない。
    #   ⚠️ 9/16の総点検は「100%同じ側」だけを拾ったので、65%のここが取り残されていた。
    #      規則は最初から「過半数で確定」なので、これは新しい判断ではなく取りこぼしの回収。
    #   ⚠️⚠️ ③は**消化済みの試合だけで数える**。未消化の枠の左右は `generate_fixtures` が
    #      機械的に付けたもの（必ず逆向き）で出典の左右ではないため、分母に入れると割合が薄まる。
    #      新潟は消化だけなら20ペア中13（65%）だが、未消化を含めると28ペア中13（46%）になり
    #      過半数に届かなくなる。**数え方で結論が変わるので、必ず消化だけで数えること。**
    "pref-niigata-1",    # 新潟1部。20ペア中13（65%）が両巡とも同じ側（新潟県サッカー協会 公式）
    # ------------------------------------------------------------------
    # ↓ 2026-09-18。goalnote/tecra の会場が入ったので②で判定した7リーグのうち6つ。
    #   （素材＝JSONの `matches[].venue`／判定＝運営フォルダ「7リーグのH_A判定②_結果_2026-09-18.md」）
    #   ⭐️ ①（ホーム試合数）は7リーグとも差が最大±3、③（両巡とも同じ側）は**7リーグとも0**。
    #      ①③はどれも「白く見える」。**②だけが7つを見分けた。**
    #   ⭐️ 「無関係の割合」と「左の割合」は完全に逆順に並ぶ＝同じことを2方向から見ているだけ。
    # 【A】セントラル開催で確定（学校のグラウンドが持ち回り会場になっている）
    "pref-iwate-1",      # 岩手1部。分母22：左7・右4・無関係11（32%）。花巻東G・盛岡誠桜Gが持ち回り
    "pref-shiga-1",      # 滋賀1部。分母36：左10・右6・無関係20（28%）。綾羽が19試合を受けて綾羽は8試合だけ
    "pref-nagasaki-1",   # 長崎1部。分母18：左4・**右5**・無関係9（22%）。右のほうが多い＝否定
    # 【B】セントラル開催ではないが「左＝持ち主」が言えない（証明できなかった）
    #      ⚠️ 「H/Aではない」ではない。出典が会場や主催の列を増やしたら再判定する。
    "pref-aichi-1",      # 愛知1部。分母44：左36・右7・無関係1（82%）。90%に届かない
    "pref-fukuoka-1",    # 福岡1部。分母52：左37・右11・無関係4（71%）。熊本65%・宮崎71%と同じ帯
    # 【C】左94%だが、**持ち主の施設で持ち主が出ない試合が実在する**ので出せない
    #      茨城1部。分母35：左33・右1・無関係1。無関係の1件は
    #      水戸啓明A × 霞ヶ浦A @SISファシリティ（同会場の残り4試合は水戸ホーリーホックユースAが左）。
    #      ⭐️ SISファシリティは **水戸ホーリーホックの施設**（一般社団法人MFA-B&S＝水戸市サッカー協会と
    #         水戸ホーリーホックが設立・2025年3月竣工）。借用している民間施設ではないことを一次資料で確認済み。
    #         出典: https://www.mito-hollyhock.net/news/p=42208/ ／ 茨城新聞 2025-03-19
    #      ⭐️ **東京との差はここ**：東京は76試合すべてで無関係0（1件も貸していない）。茨城は実際に貸している。
    #         `H` は読者への断言なので、35試合中1試合が確実に嘘になる表示は出せない。
    #      📌 半年後に「94%なのになぜ止めた」と読む人が必ず出るので、理由をここに残す。
    "pref-ibaraki-1",
    # ⚠️ 千葉（左43/43＝100%・無関係0）・東京（92%）・**北海道（左22/22＝100%・無関係0）**は
    #    **本物**なのでここに入れない。
    #    ⭐️⭐️ **「左100%は、会場欄がホーム欄のコピーなだけでは」を必ず潰すこと。** 潰し方は2つある。
    #      (a) **同じ出典の別リーグで「右」が出ているか**（千葉で使用。同じ goalnote の愛知が右7・茨城が右1）
    #      (b) ⭐️ **中立会場が存在するか**（北海道で使用・2026-09-18 に追加）。
    #          会場欄がホーム欄から自動で埋まるなら、**中立会場は原理的に出てこない**。
    #          北海道は42試合中20が学校名ですらない公共施設で、同じ会場を複数チームが使っている
    #          （ストリームスポーツ6・忠和公園5）。**出典が1本しかないリーグでも使える潰し方。**
    #    📌 北海道の補足：会場「旭川実業高校」の持ち主は **旭川実業高校2nd**（同じ旭川実業の
    #       クラブチーム `旭実FC VITA` も同リーグにいる）。この2試合はどちらを持ち主としても
    #       「無関係」にはならないので 22/22 は変わらないが、**無関係0の内訳に判断が1件入っている**。
    #    ⚠️ 照合は**サイト表記**（とわの森三愛高校・旭川実業高校2nd…）で行うこと。
    #       JSONは名寄せ後なので、**出典表記（とわの森三愛・旭川実2nd）では1件も当たらない**。
